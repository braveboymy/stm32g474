...
以及 MISRA 检查与 PC 单测。所有操作均有脚本支撑（`scripts/dev.py`），可直接执行，
也支持多步组合（`verify`）。

## 结构与多项目复用

引擎与配置分离：**引擎在 skill 目录**（可跨项目复用），**项目参数在项目根 `devtool.conf`**。

```
.pi/skills/stm32g474-devtools/
├── scripts/            # 引擎：dev.py / devtool.py / install_devtools.sh / install_hooks.sh
├── assets/             # 通用依赖：cppcheck-addons（MISRA addon）
...
升级 skill 即升级钩子；安装：`bash scripts/install_hooks.sh`）。

**新项目接入**：`bash <skill>/scripts/install_devtools.sh`（生成 devtool.conf + 钩子），
编辑 devtool.conf 即用；命令入口统一用 `<项目根>/.pi/skills/stm32g474-devtools/scripts/dev.py`。

## 前置条件

- 工具链：arm-none-eabi-gcc、cmake、ninja（引擎自动探测，无串口场景也可用）
- J-Link 软件（SEGGER 安装目录：`C:/Program Files/SEGGER/JLink_*` 或 `(x86)/SEGGER/JLink_*`，引擎自动探测取版本最大者；本机已装 JLink_V688c），目标 STM32G474RE，SWD 4MHz
...
脚本入口：`scripts/dev.py`（内部复用同目录 `devtool.py` 引擎，参数读项目根 `devtool.conf`）
...
| `dev.py misra [dir...]` | MISRA C:2012 检查（cppcheck addon） | `✅ 无 MISRA 违规` |
| `dev.py test` | core 层 PC 单测（host gcc） | `✅ PC 单测全部通过` |
...
- 分区约定：`docs/flash-partition.md`（boot 32KB / app 448KB @0x08008000 / 参数 32KB）— 变更时同步 devtool.conf
